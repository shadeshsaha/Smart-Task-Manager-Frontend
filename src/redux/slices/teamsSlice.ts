...
// OR keep types locally (safe option)
export interface Team {
  id: number;
  name: string;
}
...
// Small helper because token can be null in Redux, but thunks require string
const safeToken = (token: string | null): string => token ?? "";

/* -----------------------------------
   ASYNC THUNKS (STRICT TOKEN FIXED)
----------------------------------- */

export const fetchTeams = createAsyncThunk(
  "teams/fetchTeams",
  async (token: string | null) => {
    const teams = await getTeams(safeToken(token));
    return teams as Team[];
  }
);
...
export const addTeam = createAsyncThunk(
  "teams/addTeam",
  async ({ data, token }: { data: Partial<Team>; token: string | null }) => {
    const team = await createTeam(data, safeToken(token));
    return team as Team;
  }
);
...
export const editTeam = createAsyncThunk(
  ...
  async ({
    ...
    const team = await updateTeam(id, data, safeToken(token));
    return team as Team;
  }
);
...
export const removeTeam = createAsyncThunk(
  ...
  async ({ id, token }: { id: number; token: string | null }) => {
    await deleteTeam(id, safeToken(token));
    return id;
  }
);
...
export const fetchMembers = createAsyncThunk(
  "teams/fetchMembers",
  async ({ teamId, token }: { teamId: number; token: string | null }) => {
    const members = await getMembers(teamId, safeToken(token));
    return { teamId, members: members as Member[] };
  }
);
...
export const addMember = createAsyncThunk(
  ...
  async ({
    ...
    const member = await createMember(teamId, data, safeToken(token));
    return { teamId, member: member as Member };
  }
);
...
export const editMember = createAsyncThunk(
  ...
  async ({
    ...
    const member = await updateMember(id, data, safeToken(token));
    return member as Member;
  }
);
...
export const removeMember = createAsyncThunk(
  ...
  async ({
    ...
    await deleteMember(id, safeToken(token));
    return { id, teamId };
  }
);
...
const teamsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addMember.fulfilled, (state, action) => {
        const { teamId, member } = action.payload;
        if (!state.members[teamId]) state.members[teamId] = [];
        state.members[teamId].push(member);
      })
